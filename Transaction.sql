...
-- ACID 특성
-- 원자성(Atomicity): 하나의 트랜잭션은 모두 성공하거나 모두 실패해야 함.
-- 일관성(Consistency): 트랜잭션이 완료되면 데이터베이스 상태가 일관성을 유지해야 함.
-- 독립성(Isolation): 하나의 트랜잭션은 다른 트랜잭션에 영향을 받지 않아야 함.
-- 지속성(Durability): 트랜잭션이 성공적으로 완료되면 데이터는 영구히 저장되어야 함.
